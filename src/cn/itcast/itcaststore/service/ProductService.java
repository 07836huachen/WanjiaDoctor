...
public class ProductService {
	...
	public void addProduct(Product p) throws AddProductException {
		try {
			...
			e.printStackTrace();
			throw new AddProductException("添加医生失败");
		}
	}
	// 查找所有医生信息
	public List<Product> listAll() throws ListProductException {
		try {
			...
			e.printStackTrace();
			throw new ListProductException("查询医生失败");
		}
	}
	// 分页操作
	...
	public Product findProductById(String id) throws FindProductByIdException {
		try {
			...
			e.printStackTrace();
			throw new FindProductByIdException("根据ID查找医生失败");
		}
	}
	// 下载销售榜单
	...
	public List<Object[]> getWeekHotProduct() {
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("前台获取本周热榜医生失败！");
		}
	}
	//前台，用于搜索框根据医生姓名来模糊查询相应的医生
	public PageBean findBookByName(int currentPage, int currentCount,
		...
		try {
			...
			e.printStackTrace();
			throw new RuntimeException("前台搜索框根据医生姓名查询医生失败！");
		}
	}
	//后台系统，根据id删除商品信息
	public void deleteProduct(String id) {
		try {
			...
			throw new RuntimeException("后台系统根据id删除医生信息失败！");
		}
	}
}
